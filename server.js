...
app.post('/shipping/calculate', async (req, res) => {
  ...
  try {
    ...
    if (!data || !data[0]) return res.status(500).json({ error: 'Não foi possível calcular o frete' });
...
app.post('/pagseguro/create_order', async (req, res) => {
  ...
  if (!items?.length || !customer) return res.status(400).json({ error: 'Itens e dados do cliente obrigatórios' });
  ...
  try {
    ...
    const checkoutUrl = checkoutCodeMatch ? `https://sandbox.pagseguro.uol.com.br/v2/checkout/payment.html?code=${checkoutCodeMatch[1]}` : null;
